    ...
    lifecycle:{
      created: function(){
        ...
        this.xtag.data = {
          ...
          index: 0,
          items: [],
          buffer: 500,  //used to create margins
          renderedElements: 10,
          avgHeight:0,
          inMargin: 0,
          delta:0
...
//console.log("moving", swapCount, list.xtag.data.delta, marginDelta, list.xtag.data.index);
              ...
              for (var i = 0; i < swapCount; i++){
                var data = list.items[(list.xtag.data.index++) + list.xtag.data.xtag.data.buffer];
...
function init(){
  ...
  for(var count = 0; count < this.xtag.data.renderedElements && (idx+count)<length; count++){    
...
//console.log("DEBUG:", this.firstChild.scrollHeight, marginTop);
  ...
  if (marginTop == 0){
    avgHeight = this.xtag.data.avgHeight = (this.firstChild.scrollHeight - marginTop)  / (this.xtag.data.buffer) ;
  } else {
    avgHeight = this.xtag.data.avgHeight;
  }

  marginTop = avgHeight * idx;
  var bottomMargin = avgHeight * (length - this.xtag.data.buffer - idx);

  //console.log("total height", this.xtag.data.buffer,avgHeight, this.scrollHeight/this.xtag.data.buffer, marginTop, bottomMargin);
